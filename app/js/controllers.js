...
angular.module('clearApp.controllers', [])
	.controller('UserCtrl', ['$scope', '$route', '$timeout', '$cookieStore', 'authService', '$http', 'Base64', 'Elements_v1', 
		function ($scope, $route, $timeout, $cookieStore, authService, $http, Base64, Elements_v1) {
	...
	.controller('TvCtrl', ['$interval', '$rootScope', '$scope', 'Elements_v1', 'News', 'ClearFn', function($interval, $rootScope, $scope, Elements_v1, News, ClearFn) {
	...
	.controller('IndicatorsCtrl', ['$scope', 'Elements_v1', 'ChartsConfig', function($scope, Elements_v1, ChartsConfig) {
		$scope.charts = [];
		Elements_v1.query({'type': 'kpi'}, function(charts){
	...
	.controller('InspectionReportsCtrl', ['$location', '$scope', 'Elements_v1', '$timeout', '$filter', 'Utils', 'ClearFn', function($location, $scope, Elements_v1, $timeout, $filter, Utils, ClearFn) {
		...
		Elements_v1.query(queryType, function(docs) { 
			$scope.badges = ClearFn.badgesDisplay(query); 
		    $scope.docs = docs; 
		});
		Elements_v1.get({'type': 'ir', 'id': 'filter'}, function(filters) { 
		    $scope.filters = filters; 
		});
		
	}])
	.controller('InspectionReportCtrl', ['$scope', '$filter', '$routeParams', 'Elements_v1', function($scope, $filter, $routeParams, Elements_v1) {
		Elements_v1.get({'type': 'ir', 'id': $routeParams.id}, function(doc) {
			$scope.doc = doc;
	...
	.controller('NonComplianceReportsCtrl', ['$scope', 'ClearFn', 'Elements_v1', function($scope, ClearFn, Elements_v1) {
		Elements_v1.query({'type': 'ncr'}, function(docs) { 
		    $scope.docs = docs; 
		});
		Elements_v1.get({'type': 'ncr', 'id': 'filter'}, function(filters) { 
		    $scope.filters = filters; 
		});
		$scope.go = ClearFn.go;
	}])
	.controller('NonComplianceReportCtrl', ['$scope', '$routeParams', 'Elements_v1', '$modal', function($scope, $routeParams, Elements_v1, $modal) {
		Elements_v1.get({'type': 'ncr', 'id': $routeParams.id}, function(doc) {
			$scope.doc = doc;
		});
...
//                $log.info('Modal dismissed at: ' + new Date());
	...
	.controller('ProofsOfDeliveryCtrl', ['$location', '$scope', 'ClearFn', 'Elements_v1', function($location, $scope, ClearFn, Elements_v1) {
		Elements_v1.query({'type': 'pod'}, function(docs) { 
		    $scope.docs = docs; 
		});
		Elements_v1.get({'type': 'pod', 'id': 'filter'}, function(filters) { 
		    $scope.filters = filters; 
		});
		$scope.go = ClearFn.go;
	}])
	.controller('ProofOfDeliveryCtrl', ['$scope', '$routeParams', 'ClearFn', 'Elements_v1', function($scope, $routeParams, ClearFn, Elements_v1) {
		Elements_v1.get({'type': 'pod', 'id': $routeParams.id}, function(doc) {
			$scope.doc = doc;
		});
		$scope.go = ClearFn.go;
	}])
