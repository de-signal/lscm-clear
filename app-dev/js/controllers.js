...
angular.module('clearApp.controllers', [])
	...
	.controller('UserCtrl', ['$scope', '$route', 'toaster', '$cookieStore', '$rootScope', 'authService', '$http', 'E1', 'ClearToken',  
		function ($scope, $route, toaster, $cookieStore, $rootScope, authService, $http, E1, ClearToken) {
		var token;
		$rootScope.section = "logistic"; 
	...
	.controller('ElementsSearchCtrl', ['$scope', '$location', '$timeout', 'ClearUrl', 'Utils', 'ElmsConf', function($scope, $location, $timeout, ClearUrl, Utils, ElmsConf) {
		$timeout(function() {
...
//				    	    data: {myObj: $scope.myModelObj},
	...
	.controller('ElementModalConditionCtrl', ['$scope', '$upload', 'ClearElement', 'ClearUrl', '$modalInstance', 'condition', 'elm', function ($scope, $upload, ClearElement, ClearUrl, $modalInstance, condition, elm) {
...
//                $log.info('Modal dismissed at: ' + new Date());
	...
	.controller('WarehousesCtrl', ['$scope', '$filter', '$rootScope', '$routeParams', 'ClearUrl', 'S1', 'Warehouses', 'Utils', function($scope, $filter, $rootScope, $routeParams, ClearUrl, S1, Warehouses, Utils) {
		$rootScope.section = 'stock'; 
		...
		var r = ($routeParams.static) ? Warehouses : S1; 
		var p = ($routeParams.static) ? {} : { "type": "warehouse" }; 
		
		r.query(p, function(warehouses) {
			$scope.warehouses = warehouses; 
			$scope.loaded = true;
		});
	}])
	
	.controller('MovementsCtrl', ['$scope', '$filter', '$rootScope', '$routeParams', 'ClearUrl', 'Utils', 'MovementsConf', function($scope, $filter, $rootScope, $routeParams, ClearUrl, Utils, MovementsConf) {
		$rootScope.section = 'stock'; 
		...
		MovementsConf.get( function(config) {
			...
			listConfig.resource = ($routeParams.static) ? '30' : '3'; 
			$scope.$broadcast('event:ListInit', listConfig.id);
			ClearUrl.listReady('conf', listConfig); 
		}); 
	}])
	
	.controller('ReplenishmentsCtrl', ['$scope', '$filter', '$rootScope', '$routeParams', 'ClearUrl', 'Utils', 'ReplenishmentsConf', function($scope, $filter, $rootScope, $routeParams, ClearUrl, Utils, ReplenishmentsConf) {
		$rootScope.section = 'stock'; 
		...
		ReplenishmentsConf.get( function(config) {
			...
			listConfig.resource = ($routeParams.static) ? '31' : '3'; 
			$scope.$broadcast('event:ListInit', listConfig.id);
			ClearUrl.listReady('conf', listConfig); 
		}); 
	}])
	
	.controller('ItemsCtrl', ['$scope', '$routeParams', '$rootScope', 'ClearUrl', 'Utils', 'ItemsConf', function($scope, $routeParams, $rootScope, ClearUrl, Utils, ItemsConf) {
		$rootScope.section = 'stock'; 
		...
		ItemsConf.get( function(conf) {
			...
			listConf.name = $scope.name;  
			listConf.id = "items";
			listConf.resource = ($routeParams.static) ? '32' : '3'; 
			$scope.$broadcast('event:ListInit', listConf.id);
			ClearUrl.listReady('conf', listConf); 
		}); 
	}])
	
	.controller('ItemCtrl', ['$scope', '$routeParams', '$location', '$interval', '$timeout', '$rootScope', '$modal', 'S1', 'Utils', 'ClearUrl', 'ClearElement', 'ClearToken', 'ElmsConf', 'Item', 'ChartsConfig', function($scope, $routeParams, $location, $interval, $timeout, $rootScope, $modal, S1, Utils, ClearUrl, ClearElement, ClearToken, ElmsConf, Item, ChartsConfig) {
		$rootScope.section = 'stock'; 
		$scope.loaded = false;
		$scope.relatedActiveTab = {};
		...
		if ($routeParams.id === 'static') {
			...
			var r = S1; 
			var p = { "type": "item", "id": $routeParams.id }; 
			var imgToken = '?oauth_token=' + ClearToken.returnToken(); 
		}
	...
	.controller('WarehouseCtrl', ['$scope', '$routeParams', '$location', '$timeout', '$anchorScroll', '$modal', '$q', '$rootScope', 'S1', 'Utils', 'ClearUrl', 'ReplenishmentsConf', 'Warehouse', 'MovementsConf', 'ItemsConf', function($scope, $routeParams, $location, $timeout, $anchorScroll, $modal, $q, $rootScope, S1, Utils, ClearUrl, ReplenishmentsConf, Warehouse, MovementsConf, ItemsConf) {
		$rootScope.section = 'stock'; 
		...
		if ($routeParams.id === 'static') {
			...
			var r = S1; 
			var p = {'type': 'warehouse', "id": $routeParams.id }; 
		}
		...
		r.get(p, function(warehouse) {
			...
			for (var i in warehouse.related) {
				confs[warehouse.related[i].type].get(function(conf) { 
					...
					listsConfig[type].resource = ($routeParams.id === 'static') ? ress[type] : '3';
	...
	.controller('StockIndicatorsCtrl', ['$scope', '$routeParams', '$rootScope', 'S1', 'StockIndicators', 'ChartsConfig', function($scope, $routeParams, $rootScope, S1, StockIndicators, ChartsConfig) {
		$rootScope.section = 'stock'; 
		$scope.loaded = false;
		$scope.charts = [];
		if ($routeParams.static) {
			var r = StockIndicators; 
			var p = {}; 		
		} else {
			var r = S1; 
			var p = {'type': 'kpi'}; 
